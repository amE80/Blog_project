     ...
     <nav v-if="userStore.user" class=" w-full h-10 sticky top-0 bg-white">
        <ul class="flex items-center justify-between my-3 mx-1 sm:mx-7 md:mx-20">
         ...
         <ul class="flex items-center content-center space-x-2 sm:space-x-4">      
            ...
            <li v-if="user"  class="text-xs sm:text-sm cursor-pointer md:text-base hover:text-mainRed font-main text-gray-400 truncate w-36 " ><router-link class="" :to="{path : `/user-profile/${this.userStore.user.username}` }" active-class="text-mainRed"> {{ userStore.user.username }} </router-link></li>
        </ul>
...
export default {
  ...
  created(){
    ...
    data(){
        return{
            user: Object.keys(this.userStore.user).length
        }
    },
}
</script>
