<template>
    <main class=" mx-7 md:mx-20 font-main">
      ...
      <div v-if="!articleStore.articles.length && !articleStore.fetching_in_progress " class="text-xl ml-10 mt-10"> There is no blog </div>
  ...
  export default {
    ...
    async setup() {
      ...
      await useArticlesStore().getFavPosts(route.params.username)
    ...
    watch: {
   $route(to , from){
    this.articleStore.getFavPosts(this.$route.params.username);
    }
    },
